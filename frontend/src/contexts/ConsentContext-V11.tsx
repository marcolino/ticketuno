...
export const ConsentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [setup, setSetup] = useState(false); // true = full modal, false = banner
  ...
  useEffect(() => {
    ...
    // No consent found → open full dialog
    setSetup(true);
    setOpenDialog(true);
    setConsentInitialized(true);
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            borderRadius: 2,
            position: 'relative',
            bottom: 0, // stays visually near bottom
            p: 3,
          },
        }}
      >
        ...
        <DialogContent dividers>
          {!setup ? (
            <Typography variant="body2">
              {t(
                'We use cookies and communication tools to improve your experience. Please select your preferences.'
              )}
            </Typography>
          ) : (
            <>
