...
function GeneralSetup() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        //throw new Error("urca");
        // success → nothing to do
        setInitialStatus(status);
        setJustSaved(true);
        setTimeout(() => { // hide after 1 second
          setJustSaved(false);
        }, 1 * 1000); // TODO: to config
      } catch(error) {
        // rollback
        if (previous) {
          setStatus(previous);
        ...
        setSaveError('Failed to save'); // update error state
        toast.error(t('Settings could not be saved: {{err}}', { err: getErrorMessage(error) }));
      } finally {
        setSaving(false);
      }
    }, 0.33 * 1000); // TODO: to config
  ...
  const handleReset = () => {
    ...
    toast.warning(t('Settings have been reset'));
  };
