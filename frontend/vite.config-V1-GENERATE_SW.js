    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          // ── 7. Locales / translations ──────────────────────────────────────
          // These are served from /api/ but are static JSON files — they only
          // change on deploy, at which point the SW update cycle refreshes them.
          // Must come BEFORE the /api/ catch-all NetworkOnly rule.
          {
            urlPattern: ({ url }) =>
              /^\/api\/v\d+\/locales\//.test(url.pathname),
            handler: 'StaleWhileRevalidate',
            options: {
              cacheName: 'api-locales',
              expiration: {
                maxEntries: 20,          // languages × namespaces
                maxAgeSeconds: 7 * 24 * 60 * 60, // 1 week — refreshed on deploy anyway
              },
              cacheableResponse: { statuses: [0, 200] },
            },
          },
