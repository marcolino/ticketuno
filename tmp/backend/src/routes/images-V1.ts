...
const storage: StorageEngine = multer.diskStorage({
  destination: async (req, file, cb) => {
    //const uploadDir = path.join(__dirname, '../uploads/images'); // TODO: to config
    const uploadDir = path.join(process.cwd(), '../uploads/images'); // TODO: to config
...
router.get("/:imageId/metadata", async (req: Request, res: Response) => {
  try {
    const image = await database.getImageMetadata(req.params.imageId);
     if (!image) {
...
router.get('/:imageId', async (req: Request, res: Response) => {
  try {
    ...
    res.setHeader('Cache-Control', 'public, max-age=31536000'); // Cache for 1 year - TODO: to config
  ...
  try {
    ...
    if (image?.filepath) {
      try {
        ...
        // File might already be gone — that's fine, continue with DB cleanup
      }
    }
