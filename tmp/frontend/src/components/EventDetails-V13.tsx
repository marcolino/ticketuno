...
const EventDetails: React.FC = () => {
      ...
      renderCell: (params) => (
        <Box>
          <Typography variant="caption" sx={{ fontSize: '0.75rem' }}>
            {params.value} / {params.row.availableSeats + params.row.bookedSeats}
          </Typography>
        ...
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          {isAdmin ? (
            ...
            <Button
              size="small"
              variant="contained"
              color={params.row.availableSeats > 0 ? "success" : "error"}
            ...
            >
              {params.row.availableSeats === 0 ? t('Sold Out') : t('Choose seats')}
            </Button>
          )}
        </Box>
      ),
  ...
  const desktopColumns: GridColDef[] = [
    ...
    // Commented out status chip since we only show 'in programma' performances
    // {
    //   field: 'status',
    //   headerName: t('Status'),
    //   width: 120,
                      ...
                      <Box sx={{ 
                        ...
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                          {isAdmin ? (
                            ...
                            >
                              {performance.availableSeats === 0 ? t('Sold Out') : t('Choose seats')}
                            </Button>
                          )}
                        </Box>
                      </Box>
                      
                      {/* Second row: Available seats information */}
                      <Box sx={{ 
                        display: 'flex', 
                        justifyContent: 'space-between',
                        alignItems: 'center',
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {t('Seats available')}: {performance.availableSeats}/{performance.availableSeats + performance.bookedSeats}
                        </Typography>
                        
                        {/* Status chip commented out as requested */}
                        {/* <Chip
