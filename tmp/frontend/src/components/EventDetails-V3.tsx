...
const EventDetails: React.FC = () => {
  ...
  const handleAddPerformance = () => {
    ...
    showEditDialog(newPerformance);  // ← Pass it directly
  };

  const handleEditPerformance = (performance: EventPerformance) => {
    const editablePerformance = performanceToForm(performance);
    setPerformanceToEdit(editablePerformance);
    showEditDialog(editablePerformance);  // ← Pass it directly
  };
          ...
          <DatePicker
            ...
            slotProps={{
              textField: {
                fullWidth: true,
                required: true,
                // error: !performance?.performanceDate,
                // helperText: !performance?.performanceDate ? t('Date is required') : '',
              },
            }}
          />
  ...
  const getTotalSeats = (performance: EventPerformanceForm) => {
    return performance.availableSeats + performance.bookedSeats; // TODO ...
  };

  // Create Choose Seats button component to avoid repetition
  const ChooseSeatsButton = ({ performance, size = 'medium' }: { performance: EventPerformanceForm, size?: 'small' | 'medium' }) => {
