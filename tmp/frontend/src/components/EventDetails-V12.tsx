...
const EventDetails: React.FC = () => {
  ...
  // Popover state for performance details
  ...
  // Popover handlers
  const handlePopoverOpen = (event: React.MouseEvent<HTMLElement>, performance: EventPerformance) => {
  ...
  // Responsive DataGrid columns configuration
  const mobileColumns: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        <Box sx={{ fontSize: '0.85rem' }}>
          {dayjs(params.value).format('DD/MM')}
          <Typography variant="caption" display="block" color="text.secondary">
            {params.row.startTime ? formatTime(params.row.startTime) : ''}
          </Typography>
        </Box>
      ),
    },
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Tooltip title={t('Details')}>
          <IconButton
            size="small"
            onClick={(e) => handlePopoverOpen(e, params.row)}
          >
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          {isAdmin ? (
            ...
            >
              {params.row.availableSeats === 0 ? t('Sold') : t('Book')}
            </Button>
          )}
        </Box>
      ),
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          {params.value}
          <Typography variant="caption" display="block" color="text.secondary">
            /{params.row.bookedSeats + params.value}
          </Typography>
        </Box>
      ),
    },
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper elevation={3} sx={{ 
        ...
        {performances.length === 0 ? (
          ...
          <>
            {/* Mobile/Tablet compact card view as alternative to DataGrid */}
            <Hidden mdUp>
                    ...
                    <CardContent sx={{ 
                      ...
                      <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, mt: 0.5, alignItems: 'center' }}>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {performance.availableSeats} {t('available')}
                          </Typography>
                        </Box>
                      </Box>
                      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                        <Tooltip title={t('More details')}>
                          <IconButton
                            size="small"
                            onClick={(e) => handlePopoverOpen(e, performance)}
                          >
                            <InfoIcon />
                          </IconButton>
                        </Tooltip>
                        {isAdmin ? (
                          ...
                          <Button
                            ...
                            sx={{ minWidth: 70 }}
                          >
                            {performance.availableSeats === 0 ? t('Sold') : t('Book')}
                          </Button>
                        )}
                      </Box>
                    </CardContent>
            ...
            <Hidden mdDown>
              <Box sx={{ 
                ...
                '& .MuiDataGrid-row': {
                  maxHeight: { xs: 40, sm: 52 } + '!important',
                  minHeight: { xs: 40, sm: 52 } + '!important',
                }
              }}>
                <DataGrid
                  rows={performances}
          ...
          {selectedPerformance && (
            <Box>
              <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
                {formatDate(selectedPerformance.performanceDate)}
              </Typography>
              <Table size="small">
                <TableBody>
                  <TableRow>
                    <TableCell sx={{ border: 'none', py: 0.5, fontWeight: 'medium' }}>
                      {t('Time')}
                    </TableCell>
                    <TableCell sx={{ border: 'none', py: 0.5 }}>
                      {formatTime(selectedPerformance.startTime)}
                      {selectedPerformance.endTime && ` - ${formatTime(selectedPerformance.endTime)}`}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell sx={{ border: 'none', py: 0.5, fontWeight: 'medium' }}>
                    ...
                    <TableCell sx={{ border: 'none', py: 0.5 }}>
                      <Chip
                        label={t(selectedPerformance.status)}
                        color={getStatusColor(selectedPerformance.status)}
                        size="small"
                      />
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    <TableCell sx={{ border: 'none', py: 0.5 }}>
                      {selectedPerformance.availableSeats} / {selectedPerformance.availableSeats + selectedPerformance.bookedSeats}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell sx={{ border: 'none', py: 0.5, fontWeight: 'medium' }}>
                    ...
                    <TableCell sx={{ border: 'none', py: 0.5 }}>
                      {selectedPerformance.bookedSeats}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
                ...
                {isAdmin ? (
                  <>
                    <Button
                      ...
                      startIcon={<EditIcon />}
                      onClick={() => {
                        handleEditPerformance(selectedPerformance);
                        handlePopoverClose();
                      }}
                    >
                      {t('Edit')}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        handleDeletePerformance(selectedPerformance.id);
                        handlePopoverClose();
                      }}
                    >
                      {t('Delete')}
                  ...
                  <Button
                    ...
                    variant="contained"
                    onClick={() => {
                      handleBookPerformance(selectedPerformance.id);
                      handlePopoverClose();
                    }}
                    disabled={selectedPerformance.availableSeats === 0 || selectedPerformance.status !== 'scheduled'}
                  >
                    {selectedPerformance.availableSeats === 0 ? t('Sold Out') : t('Book Now')}
                  </Button>
                )}
              </Box>
            </Box>
