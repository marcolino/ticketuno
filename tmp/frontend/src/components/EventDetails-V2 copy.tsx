...
const EventDetails: React.FC = () => {
  ...
  // });
  const performanceToForm = (
    performance: EventPerformance
  ): EventPerformanceForm => ({
    id: performance.id,
    eventId: performance.eventId,
    performanceDate: performance.performanceDate
      ? dayjs(performance.performanceDate)
      : null,
    startTime: performance.startTime
      ? dayjs(`1970-01-01T${performance.startTime}`)
      : null,
    endTime: performance.endTime
      ? dayjs(`1970-01-01T${performance.endTime}`)
      : null,
    status: performance.status,
    availableSeats: performance.availableSeats ?? 0,
    bookedSeats: performance.bookedSeats ?? 0,
  ...
  const handleAddPerformance = () => {
    const nextDay = performances.length === 0
      ? event?.openingDate ? dayjs(event.openingDate) : dayjs()
      : dayjs(performances
    ...
    const newPerformance: EventPerformanceForm = {
      id: undefined,
      eventId: id!,
      performanceDate: nextDay,
      startTime: event?.typicalStartTime
        ? dayjs(`1970-01-01T${event.typicalStartTime}`)
        : null,
      endTime: event?.typicalEndTime
        ? dayjs(`1970-01-01T${event.typicalEndTime}`)
        : null,
      status: 'scheduled',
      availableSeats: 0,
      bookedSeats: 0,
    ...
    //});
    setPerformanceToEdit(newPerformance);
    showEditDialog();
    //showEditDialog(newPerformance);
  };

  // const handleEditPerformance = (performance: EventPerformance) => {
  //   //setPerformanceToEdit(performanceToForm(performance));
  //   // setEditDialogOpen(true);
  //   const editablePerformance = performanceToForm(performance);
  //   //setPerformanceToEdit(newPerformance);
  //   //showEditDialog();
  //   showEditDialog(editablePerformance);
  // };
  const handleEditPerformance = (performance: EventPerformance) => {
    setPerformanceToEdit(performanceToForm(performance)); // set state
    showEditDialog(); // reads from state
  };
  ...
  const showEditDialog = () => {
    ...
    showDialog({
      title: performanceToEdit.id ? t('Edit Performance') : t('Add Performance'),
      content: (
        <Stack spacing={3} sx={{ mt: 2 }}>
          ...
          <DatePicker
            ...
            onChange={(value: Dayjs | null) =>
              setPerformanceToEdit(prev => prev ? { ...prev, performanceDate: value } : prev)
            }
            minDate={dayjs()}
            slotProps={{
          ...
          <TimePicker
            ...
            onChange={(value: Dayjs | null) =>
              setPerformanceToEdit(prev => prev ? { ...prev, startTime: value } : prev)
            }
            ampm={false}
          ...
          <TimePicker
            ...
            onChange={(value: Dayjs | null) =>
              setPerformanceToEdit(prev => prev ? { ...prev, endTime: value } : prev)
            }
            ampm={false}
            slotProps={{ textField: { fullWidth: true, required: true } }}
          />
      ...
      cancelText: t('Cancel'),
      confirmText: t('Save'),
      onConfirm: handleSavePerformance,
    });
  };

  // const showEditDialog = (performance: EventPerformanceForm) => {
  //   /* Edit/Add Performance Dialog */
  //   showDialog({
  //     //maxWidth: "sm",
  //     title: performance?.id ? t('Edit Performance') : t('Add Performance'),
  //     content: (
  //       <Stack spacing={3} sx={{ mt: 2 }}>
  //         <DatePicker
  //           label={t('Performance Date')}
  ...
  const columns: GridColDef<EventPerformance>[] = [
    {
      field: 'performanceDate',
      headerName: t('Date'),
      width: 120,
      renderCell: (params) =>
        params.row.performanceDate
          ? formatDate(params.row.performanceDate)
          : '',
    },
    {
      field: 'startTime',
