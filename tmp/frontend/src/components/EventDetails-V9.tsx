...
import dayjs from 'dayjs';
import 'dayjs/locale/it'; // TODO: do we need these here too, after App.tsx?
...
const EventDetails: React.FC = () => {
  ...
  // DataGrid states - use GridRowId[] instead of GridRowSelectionModel
  ...
  dayjs.locale('it'); // Set your locale - TODO: 'it' to current locale, and in useEffect...
  ...
  const loadEvent = useCallback(async () => {
    try {
      ...
      //const perfResponse = response.data.performances;  //await eventApi.getPerformances(id!);
  ...
  const reloadEvent = async () => {
    const scrollPosition = window.pageYOffset; // Save current scroll position
    await loadEvent();
    setTimeout(() => window.scrollTo(0, scrollPosition), 0); // Restore scroll position after load
  }
  ...
  // Date/Time handlers for pickers - FIXED for MUI X v8+
  const handlePerformanceDateChange = (newDate: Dayjs | null) => {
  ...
  const formatDate = (dateString: string) => {
    ...
    return date.format('L'); // Locale-aware long date format
  };
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 2 }}>
            <DatePicker<Dayjs>
              ...
              onChange={(newDate) => handlePerformanceDateChange(newDate)}
              format="L"
              minDate={dayjs()}
              slotProps={{
                textField: {
