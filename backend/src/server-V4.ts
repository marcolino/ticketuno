...
app.use((req, res, next) => {
  ...
  if (maintenanceMode) {
    ...
    // non-API request → serve maintenance page
...
app.get(`${prefix}/locales/:lng/:ns.json`, (req: Request, res: Response) => {
  ...
  fs.access(filePath, fs.constants.F_OK, (err) => {
    if (err) return res.status(404).json({ error: 'Translation file not found' });
...
// === Frontend SPA or Maintenance ===
if (process.env.NODE_ENV === 'production') {
  const publicDir = frontendDistDir;
